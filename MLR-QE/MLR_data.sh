#/bin/bash RR data preparation for training the MLR
...
do
  ...
  do
    paste  $BASEDIR/data/features/${setname}_CH_${ch}.rank $BASEDIR/data/features/${setname}_CH_${ch}_${Feat}.feat | sed -n "${segN}p" \
           | awk -v "qid=$qid" '{printf("%d qid:%s",$1,qid) ; for(i=2;i<=NF;i++) printf(" %d:%.5f",i-1,$i); printf("\n")}';
  done

done > $BASEDIR/data/MLR_${setname}_${Feat}.data
